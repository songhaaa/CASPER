...
class Domain_adaption_model(nn.Module):
   def __init__(self,hidden_1,hidden_2,hidden_3,hidden_4,num_of_class,low_rank,max_iter,upper_threshold,lower_threshold, temp=1):
       ...
       self.temp=temp
       self.qhat = (torch.ones([1, num_of_class], dtype=torch.float) / num_of_class).cuda()
   ...
   def forward(self,source,target,source_label, target_label):
       ...
       distances = torch.cdist(feature_source_g, self.P.cuda())
       weights = torch.exp(-distances) * source_label
       weights = weights / weights.sum(dim=1, keepdim=True)
       adjusted_labels = weights * source_label
       self.P = torch.matmul(torch.inverse(torch.diag(adjusted_labels.sum(axis=0)) + torch.eye(self.num_of_class).cuda()),torch.matmul(adjusted_labels.T, feature_source_g))
       self.stored_mat=torch.matmul(self.V,self.P.T)
       ...
       return source_predict, target_predict, feature_source_f,feature_target_f,sim_matrix,sim_matrix_target,source_label_feature,target_label_feature
   ...
   def update_qhat(self, probs, qhat, momentum, qhat_mask=None, epsilon=1e-5):
       if qhat_mask is not None:
           masked_probs = probs.detach() * qhat_mask.detach().unsqueeze(dim=-1)
           valid_mask = qhat_mask.sum()
           if valid_mask > 0:
               mean_prob = masked_probs.sum(dim=0) / valid_mask
           else:
   ...
   def predict_confidence(self,test_features, tau, sharpen=1):
       self.eval()
       feature_target_f=self.fea_extrator_f(test_features)

       delta_logits = torch.log(self.qhat)
       delta_logits = delta_logits.expand(feature_target_f.size(0), -1)
       test_logit = torch.matmul(torch.matmul(self.U, feature_target_f.T).T, self.stored_mat.cuda())
       test_logit = test_logit - tau * delta_logits
       test_cluster=torch.nn.functional.softmax(test_logit, dim=1)
       if sharpen==1:
           test_cluster=self.sharpen(test_cluster,self.temp)
       test_predict=torch.zeros_like(test_cluster)
